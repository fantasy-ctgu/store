...
public class SellerDaoImpl implements IBaseDao<String, User>{
	...
	public boolean doInsert(User v) {
		String sql = "insert into seller_info(username,password,name,sex,age,address) values(?,?,?,?,?,?)";
		try {
	...
	public boolean doUpdate(User v) {
		String sql = "update seller_info set password = ?,name = ?,sex = ?,age = ?,address = ? WHERE username = ?";
		try {
			this.ps = con.prepareStatement(sql);
